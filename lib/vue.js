    ...
    */
	...
	function _createVNode(
		...
		const shapeFlag = isString(type)
			...
			: isSuspense(type)
				...
				: isTeleport(type)
					? 64 /* TELEPORT */
					: isObject(type) ? 4 /* STATEFUL_COMPONENT */ : isFunction(type) ? 2 /* FUNCTIONAL_COMPONENT */ : 0;
	...
	function generate(ast, options = {}) {
		...
		if (useWithBlock) {
			...
			if (hasHelpers) {
				push(
					`const { ${ast.helpers.map((s) => `${helperNameMap[s]}: _${helperNameMap[s]}`).join(', ')} } = _Vue`
				);
				push(`\n`);
				newline();
			}
	...
	const transformFor = createStructuralDirectiveTransform('for', (node, dir, context) => {
		...
		return processFor(node, dir, context, (forNode) => {
			...
			const fragmentFlag = isStableFragment
				? 64 /* STABLE_FRAGMENT */
				: keyProp ? 128 /* KEYED_FRAGMENT */ : 256 /* UNKEYED_FRAGMENT */;
	...
	function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
		...
		const slotFlag = hasDynamicSlots
			? 2 /* DYNAMIC */
			: hasForwardedSlots(node.children) ? 3 /* FORWARDED */ : 1 /* STABLE */;
